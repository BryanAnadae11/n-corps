...
def home(request):
	return render(request, 'ncorpsapp/home.html')

def about(request):
	return render(request, 'ncorpsapp/about.html')

def blockchainfund(request):
	return render(request, 'ncorpsapp/blockchainfund.html')

def liquidtoken(request):
	return HttpResponse('Liquid Token will be launched soon')

def bitcointradingfund(request):
	return render(request, 'ncorpsapp/bitcointradingfund.html')

def earlystagetoken(request):
	return HttpResponse('Early Stage Token will be launched soon')

def stakingfund(request):
	return render(request, 'ncorpsapp/stakingfund.html')

def energies(request):
	return render(request, 'ncorpsapp/energies.html')

def sharestrading(request):
	return render(request, 'ncorpsapp/sharestrading.html')

def agriculturalfund(request):
	return render(request, 'ncorpsapp/agriculturalfund.html')

def nonfarmpayroll(request):
	return render(request, 'ncorpsapp/nonfarmpayroll.html')

def renewableenergy(request):
	return render(request, 'ncorpsapp/renewableenergy.html')

def investmentmanagement(request):
	return render(request, 'ncorpsapp/investmentmanagement.html')

def financialservice(request):
	return render(request, 'ncorpsapp/financialservice.html')

def portfoliomanagementservice(request):
	return render(request, 'ncorpsapp/portfoliomanagementservice.html')

def financialadvising(request):
	return render(request, 'ncorpsapp/financialadvising.html')

def pressrelease(request):
	return render(request, 'ncorpsapp/about.html')

def faqs(request):
	return render(request, 'ncorpsapp/faqs.html')

def terms(request):
	return render(request, 'ncorpsapp/terms.html')

def privacy(request):
	return render(request, 'ncorpsapp/privacy.html')

def otherpayment(request):
	return HttpResponse('Sorry but this method of payment is not available to you at the moment. Please, try using any available cryptocurrency of your choice, or contact support@n-crg.org for help')

def contact(request):
	return render(request, 'ncorpsapp/contact.html')
...
def main_view(request, *args, **kwargs):
	...
	return render(request, 'ncorpsapp/main.html')
...
def signin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				template= render_to_string('ncorpsapp/loginAlert.html', {'name':username})
	...
	context={}
	return render(request, 'ncorpsapp/signin.html')
...
def signup(request):
	...
	if form.is_valid():
		if client_id is not None:
			...
			referral_template= render_to_string('ncorpsapp/referalsignupmail.html', {'name':recommended_by_client_name, 'refereed':username})
		...
		template= render_to_string('ncorpsapp/welcomeEmail.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
			'Welcome to Capital Impact Invest',
			plain_message,
			settings.EMAIL_HOST_USER,
			[email],
		...
		second_template= render_to_string('ncorpsapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		second_plain_message= strip_tags(second_template)
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with Capital Impact Invest!",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@n-crg.org'])
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'ncorpsapp/signup.html', context)
...
def dashboard(request):
	if request.user.is_staff:
		...
		total_deposit= float(client_deposit) + float(client_withdrawal)
	...
	'client_withdrawal':client_withdrawal, 'client_code':client_code, 'total_deposit': total_deposit, 'time_left':time_left}
	return render(request, 'ncorpsapp/dashboard.html', context)

@login_required(login_url='signin')
def plans(request):
	plans= Plan.objects.all()
	context= {'plans':plans}
	return render(request, 'ncorpsapp/plans.html', context)
...
def notifications(request):
	...
	return render(request, 'ncorpsapp/notifications.html', context)
...
def admindashboard(request):
	...
	return render(request, 'ncorpsapp/adminpage.html', context)
	...
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
    			...
    			"success_url": "https://www.n-crg.org/dashboard",
    			#our success url will direct us to the get_payment_status view for balance top ups
    			"cancel_url": "https://www.n-crg.org/dashboard"
    		})
			headers={'x-api-key':'JDB5ZY2-7154GQ5-PTP5D84-7ZGS6NE', 'Content-Type': 'application/json'}
			...
			Payment_id.objects.create(
				client=client,
				payment_id= '568-8534-xxx-25674',
				price_amount= price_amount,
				investment_plan=investment_plan
				)
			template= render_to_string('ncorpsapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
			plain_message= strip_tags(template)
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@n-crg.org'])
			except BadHeaderError:
				pass
			return redirect(generated_link)

	context={'packages':packages}
	return render(request, 'ncorpsapp/deposit.html', context)
...
def btc_deposit(request):
	...
	if request.method == 'POST':
		try:
			send_mail(client_name, "A client with username: {} has made a deposit of ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@n-crg.org'])
		except:
			pass
	...
	return render(request, 'ncorpsapp/btc_deposit.html', context)
...
def usdt_deposit(request):
	...
	if request.method == 'POST':
		...
		return HttpResponse('Deposit Awaiting Confirmation')
	context= {'client':client, 'pay_wallet':pay_wallet, 'last_payment_info':last_payment_info, 'wallet_address':wallet_address}
	return render(request, 'ncorpsapp/usdt_deposit.html', context)
...
def ltc_deposit(request):
	...
	if request.method == 'POST':
		...
		return HttpResponse('Deposit Awaiting Confirmation')
	context= {'client':client, 'pay_wallet':pay_wallet, 'last_payment_info':last_payment_info, 'wallet_address':wallet_address}
	return render(request, 'ncorpsapp/ltc_deposit.html', context)
'''
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@n-crg.org'])

				except BadHeaderError:
					return HttpResponse('Something went wrong, please try again later')

				return HttpResponse('Kindly wait as we process your withdrawal')
		...
		if withdrawal_option == 'profit' and float(client_profit) > 10:
			...
			if client_profit_balance < 0:
				...
				try:
					...
					return HttpResponse('Something went wrong, please try again later')
				messages.success(request, "Kindly wait as we process your withdrawal")

		if withdrawal_option == 'profit' and float(client_profit) <=10:
			messages.error(request, "Your profit is too low for this withdrawal" )
	context={}
	return render(request, 'ncorpsapp/withdrawal.html', context)
...
def history(request):
	...
	return render(request, 'ncorpsapp/history.html', context)
...
def pending_deposit(request):
	...
	return render(request, 'ncorpsapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'ncorpsapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'ncorpsapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'ncorpsapp/completed_transaction.html', context)
...
def myreferals(request):
    ...
    return render(request, 'ncorpsapp/referralprofiles.html', context)
...
def confirm_withdrawal(request):
	...
	return render(request, 'ncorpsapp/confirmwithdrawal.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('ncorpsapp/withdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('ncorpsapp/declineWithdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def confirm_deposit(request):
	...
	return render(request, 'ncorpsapp/confirmdeposit.html', context)
...
def update_payment(request, pk):
	...
	if float(payment_info_investment_plan) == float(0.12):
		packageName= 'STARTER PLAN'
	if float(payment_info_investment_plan) == float(0.25):
		packageName= 'STANDARD PLAN'
	if float(payment_info_investment_plan) == float(0.40):
		packageName= 'PREMIUM PLAN'
	if float(payment_info_investment_plan) == float(0.50):
		packageName= 'EXCLUSIVE PLAN'
	if float(payment_info_investment_plan) == float(0.65):
		packageName= 'ULTIMATE PLAN'
	template= render_to_string('ncorpsapp/confirmDepositEmail.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def account_settings(request):
	...
	context= {"form":form}
	return render(request, 'ncorpsapp/account_settings.html', context)
...
def transfer_funds(request):
		...
		if float(client_balance) > float(amount):
			try:
				...
				if destination_client_account_info.username != client_username:
					...
					template= render_to_string('ncorpsapp/creditMail.html', {'name':destination_client_account_info.username,
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					debit_template= render_to_string('ncorpsapp/debitMail.html', {'name':client_username,
					...
					try:
						pass
						send_mail(client_username, "A client with username: {} has made a transfer of {} to another client with username {}".format(client_username, amount, destination_client_account_info.username),settings.EMAIL_HOST_USER, ['info@n-crg.org'])
					except BadHeaderError:
						pass
						return HttpResponse('Something went wrong, please try again later')
	...
	context= {}
	return render(request, 'ncorpsapp/transferFunds.html', context)
...
def video_page(request):
	...
	return render(request, 'ncorpsapp/videoPage.html', context)

@login_required(login_url='signin')
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('ncorpsapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
...
def reinvest(request):
	...
	if request.method == 'POST':
		...
		if (float(client_bal) > float(amount)):
			...
			template= render_to_string('ncorpsapp/reinvestmail.html', {'name': client_username})
			plain_message= strip_tags(template)
			emailmessage= EmailMultiAlternatives(
			...
			emailmessage.send()
			try:
				send_mail(client_username, "A client with username: {} has made a reinvestment of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@n-crg.org'])
			except BadHeaderError:
				pass
	...
	context= {'packages': packages}
	return render(request, 'ncorpsapp/reinvest.html', context)
